...
const Grid = (props) => {
  const [correctGrid, setCorrectGrid] = useState(
    formCorrectGrid(props.gridSize)
  );
  ...
  useEffect(() => {
    const handleKeyboardInput = (event) => {
      switch (event.key) {
        case "ArrowUp":
          if (
            props.empty[0] + 1 < Math.sqrt(props.gridSize) &&
            props.grid[props.empty[0] + 1][props.empty[1]] !== "0"
          ) {
        ...
        case "ArrowLeft":
          if (
            props.empty[1] + 1 < Math.sqrt(props.gridSize) &&
            props.grid[props.empty[0]][props.empty[1] + 1] !== "0"
          ) {
  ...
  const updateGrid = (rowIndex, colIndex) => {
    ...
    for (let i = 0; i < props.grid.length; i++) {
      for (let j = 0; j < props.grid[0].length; j++) {
        if (props.grid[i][j] === "0") {
          if (
            (Math.abs(rowIndex - i) === 1 && colIndex === j) ||
            (Math.abs(colIndex - j) === 1 && rowIndex === i)
          ) {
