...
function move(grid, direction) {
  ...
  const [collisionsGrid, scoreIncrement] = checkCollisions(
    updatedGridCopy,
    direction
  );
  const gridChanged = !collisionsGrid.every((row, i) =>
    row.every((cell, j) => cell.value === gridBeforeMove[i][j].value)
  );
  if (gridChanged) {
...
function addPiece(grid) {
  let zeroIndices = grid.map((_, row) =>
    grid.map((_, col) =>
      grid[row][col].value === 0 ? { x: col, y: row } : null
    )
  );
...
function checkCollisions(grid, direction) {
  ...
  if (direction === "up" || direction === "left") {
    [gridUpdated, scoreIncrement] = checkLeftCollisions(
      grid,
      direction,
      scoreIncrement
    );
    ...
    [gridUpdated, scoreIncrement] = checkRightCollisions(
      grid,
      direction,
      scoreIncrement
    );
